...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            #self.epsilon = math.cos(self.t * 0.01) -- A+,A
            self.epsilon = math.cos(self.t * 0.007)
    ...
    def createQ(self, state):
        ...
        if self.learning:
            if not state in self.Q.keys():
                self.Q[state] = {state_init: 0.0 for state_init in self.valid_actions}
                print self.Q[state]
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if self.epsilon > random.random():
                ...
                maxQ_state = self.get_maxQ(state)
                maxQ_actions = [key for key in self.Q[state].keys() if self.Q[state][key] == maxQ_state]
...
def run():
    ...
    sim = Simulator(env, display = True, update_delay=0.001, log_metrics=True, optimized = True)
